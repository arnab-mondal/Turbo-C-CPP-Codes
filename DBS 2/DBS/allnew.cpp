...
char valkey()//for setting value of x2 and y2 as per the key pressed
{
   char key;
   key = getch();
   clrscr();
...
void copydata()
{
   ...
   for(j=1;j<13;j++)
   {
      ...
      while(obj)
      {
         ...
         //if(i == x1 && j==y1 && input[0]!='\0')// && flag ==0)
         //if(k+1 == x1 && l+1 == y1 && input[0]!='\0')
         if(ex1==x1 && ey1==y1 && input[0]!='\0')
         {

         	//gotoxy(20,1);
            printf("\n i=%d x1=%d k=%d j=%d y1=%d l=%d %s \n",i,x1,k,j,y1,l,input);
            //flag = 1;
         }
      }
      obj.close();
